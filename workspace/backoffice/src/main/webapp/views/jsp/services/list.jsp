...
<head>
  ...
  <script>
	...
	$(document).ready(function() {
		//var vdata = [];
	    var employeeTable = $('#servicesTable').DataTable( {
              	...
              	if(data.statusCode = '20000') {
      		    	...
      		    	if(dataList.length > 0) {
                  		...
                  		$.each(dataList, function(i, item){
                  			...
                  			arrary[0] = (i);
                  			arrary[1] = ((dataList[i].sid!=null)?dataList[i].sid:"");

                  			var name = ((dataList[i].name!=null)?dataList[i].name:"");
    		    			var nameHtml = "<span ";
    		    			nameHtml += "sid='"+dataList[i].sid+"' >";
    		    			nameHtml += "<a class=\"services-name\" href='javascript:void(0);' onclick='detailIfno(this)'";
    		    			nameHtml += ">"+name+"</a></span>";
    		    			
                  			arrary[2] = nameHtml;
                  			arrary[3] = ((dataList[i].startTimeStr!=null)?dataList[i].startTimeStr:"");
                  			arrary[4] = ((dataList[i].description!=null)?dataList[i].description:"");
                  			tableData[i] = arrary;
                  		});
                  		employeeTable = $("#servicesTable").dataTable();   
	...
	function detailIfno(which){
		var sid = $(which).parent().attr("sid");
		window.location.href = "services/edit?sid=" + sid;
	}
  </script>
