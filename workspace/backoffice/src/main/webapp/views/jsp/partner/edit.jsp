                  ...
                  <div class="col-lg-12">
                      <section class="panel">
                          ...
                          <div class="panel-body">
                              <div class="form">
                                  <form class="form-validate form-horizontal" id="partnerForm" method="post" action="partner/update">
                                      <input class="form-control" id="pid" name="pid" type="hidden" value="${partner.pid}" />
                                      <div class="form-group imageUploadArea">
                                          <label for="cname" class="control-label col-lg-2">合作伙伴图片<span class="required">*</span></label>
                                          <div class="col-lg-10">
											  <img id="partnerImg" class="user-image-partner" src="${partner.imgUrl}" >
                                              <label>点击图片区域选择上传图片</label>
											  <input id="imgFile" name="imgFile" class="user-image-input" accept="image/*" type="file"/>
											  <input id="imgUrl" name="imgUrl" type="hidden" value="${partner.imgUrl}"/>
											  <button id="imgUpload" class="btn " type="button">使用新图片</button>
                                          </div>
                                      </div>
                                      <div class="form-group ">
                                          <label for="cname" class="control-label col-lg-2">合作伙伴名称<span class="required">*</span></label>
                                          <div class="col-lg-10">
                                              <input class="form-control" id="name" name="name" value="${partner.name}" minlength="5" type="text" required />
                                          </div>
                                      </div>
                                      <div class="form-group ">
                                          <label for="ccomment" class="control-label col-lg-2">合作伙伴描述</label>
                                          <div class="col-lg-10">
                                              <textarea class="form-control " id="description" name="description" required >${partner.description}</textarea>
                                          </div>
                                      </div>
                                      <div class="form-group">
...
<!--                                               <button class="btn btn-default" type="button">取消</button> -->
  ...
  $(document).ready(function() {
	  ...
	  $.imageUploadControl("partnerImg","imgFile","imgUrl","imgUpload","partner/uploadImg");
	  
      $("#partnerForm").on("submit", function(){
    	  var form = this;
    	  modalConfirm("是否确认修改信息",function(){
   		   var dataPara = JSON.stringify($("#partnerForm").serializeObject()); 
   		   $.ajax({
   		      url : form.action,
   		      type : form.method,
